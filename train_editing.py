...
if __name__ == '__main__':
    ...
    parser.add_argument('--update_extra_interval', type=int, default=64, help="iter interval to update extra status (only valid when using --cuda_ray)")
    ...
    parser.add_argument('--prior_mesh', type=str, default='./data/doll/mesh_sdf/doll_sdf_g_30k.ply')
    parser.add_argument('--prior_mask_mesh', type=str, default='./data/doll/mesh_sdf/doll_sdf_g_30k_hat_edit_area.ply')
    parser.add_argument('--mesh_mask_info', type=str, default='./data/doll/mask_sdf/doll_sdf_g_30k_hat_edit_area.npy')
    ...
    parser.add_argument('--backbone', type=str, default='mesh_neus', help="nerf backbone, choose from [mesh, mesh_neus, grid, vanilla]")
    ...
    parser.add_argument('--radius_range', type=float, nargs='*', default=[1.4, 1.6], help="training camera radius range")
    ...
    parser.add_argument('--light_theta', type=float, default=70, help="default GUI light direction in [0, 180], corresponding to elevation [90, -90]")
    ...
    if opt.test:
        ...
        trainer = Trainer('df', opt, model, guidance, background_model, opt, device=device, workspace=opt.workspace, fp16=opt.fp16,
        ...
        trainer = Trainer('df', opt, model, guidance, background_model, opt_back, device=device, workspace=opt.workspace,
                          ema_decay=False, fp16=opt.fp16, use_checkpoint=opt.ckpt,
                          eval_interval=opt.eval_interval, scheduler_update_every_step=True)


        valid_loader = SphericalSamplingDataset(opt, device=device, R_path=opt.R_path, type='test', H=opt.H, W=opt.W, size=4).dataloader()
