...
class _composite_rays_train_sdf(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sdf, rgbs, deltas, rays, T_thresh=1e-4):
        ''' composite rays' rgbs, according to the ray marching formula.
        Args:
            rgbs: float, [M, 3]
            sdf: float, [M,]
        ...
        weights_sum = torch.empty(N, dtype=sdf.dtype, device=sdf.device)
        depth = torch.empty(N, dtype=sdf.dtype, device=sdf.device)
        image = torch.empty(N, 3, dtype=sdf.dtype, device=sdf.device)

        _backend.composite_rays_train_forward_sdf(sdf, rgbs, deltas, rays, M, N, T_thresh, weights_sum, depth, image)

        ctx.save_for_backward(sdf, rgbs, deltas, rays, weights_sum, depth, image)
        ctx.dims = [M, N, T_thresh]
    ...
    def backward(ctx, grad_weights_sum, grad_depth, grad_image):
        ...
        sdf, rgbs, deltas, rays, weights_sum, depth, image = ctx.saved_tensors
        M, N, T_thresh = ctx.dims

        grad_sigmas = torch.zeros_like(sdf)
        grad_rgbs = torch.zeros_like(rgbs)

        _backend.composite_rays_train_backward_sdf(grad_weights_sum, grad_image, sdf, rgbs, deltas, rays, weights_sum,
                                               image, M, N, T_thresh, grad_sigmas, grad_rgbs)

        return grad_sigmas, grad_rgbs, None, None, None


composite_rays_train_sdf = _composite_rays_train_sdf.apply
