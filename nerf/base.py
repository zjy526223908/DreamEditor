...
# class ImplicitSurfaceTcnn(nn.Module):
#     def __init__(self,
#                  W=64,
#                  D=2,
#                  skips=[4],
...
#
#
#         per_level_scale = np.exp2(np.log2(2048 * obj_bounding_size / 16) / (16 - 1))
#         print(per_level_scale)
...
#         input_ch = 32
#
#         # self.embed_fn, input_ch = get_embedder(6, 3)
#
...
class ImplicitSurface(nn.Module):
    def __init__(self,
        ...
        # per_level_scale = np.exp2(np.log2(2048 * 1.0 / 16) / (16 - 1))
        # print(per_level_scale)
        # self.embed_fn = tcnn.Encoding(
        #     n_input_dims=3,
        #     encoding_config={
...
class ImplicitSurface_Mesh(nn.Module):
    def __init__(self,
                 W=256,
                 D=8,
                 skips=[4],
        ...
        self.skips = skips
        # self.embed_fn, input_ch = get_encoder('tiledgrid', input_dim=3, log2_hashmap_size=16,desired_resolution=2048)
...
class RadianceNet_Mesh(nn.Module):
    def __init__(self,
                 D=4,
                 W=256,
                 skips=[],
...
# class RadianceNetTcnn(nn.Module):
#     def __init__(self,
#                  D=2,
#                  W=64,
#                  skips=[],
...
#
#         # self.embed_fn, input_ch = get_embedder(6)
#
#         # self.embed_fn = tcnn.Encoding(
#         #     n_input_dims=3,
#         #     encoding_config={
...
#                 },
#             )
#             input_ch_views = self.embed_fn_view.n_output_dims
#
#             in_dim_0 = input_ch_views + W_geo_feat + 3 + 3
#         else:
#             in_dim_0 = input_ch_pts + W_geo_feat
#
#         self.rgb_network = tcnn.Network(
#             n_input_dims=in_dim_0,
#             n_output_dims=3,
#             network_config={
#                 "otype": "FullyFusedMLP",
#                 "activation": "ReLU",
...
class MeshRadianceNet(nn.Module):
    def __init__(self,
                 D=4,
                 W=256,
                 skips=[],
        ...
        self.use_view_dirs = use_view_dirs
        self.embed_fn, input_ch_pts = get_embedder(embed_multires)
        # self.embed_fn, input_ch_pts = get_encoder('tiledgrid', input_dim=3, log2_hashmap_size=16, desired_resolution=2048)
        if use_view_dirs:
            self.embed_fn_view, input_ch_views = get_embedder(embed_multires_view)
            in_dim_0 = input_ch_pts + input_ch_views + 3 + W_geo_feat
        else:
